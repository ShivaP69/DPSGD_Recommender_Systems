...
def train(args):
    ...
    # create data loader to generate train/test data loader (we have both simple train/test data and data loader version)
    train_loader,test_loader, train_data, test_data, all_movieIds= create_data_loader.create_data_loader(data)
    ...
    for user_id in user_interacted_items:
      ...
      interacted_distr[user_id] =Calibration.compute_genre_distr(interacted_items)
    ...
    user_indices= read_user_indices(test_user_item_dict, user_interacted_items, all_movieIds, ratio) # reading user indices
    ...
    for epoch in range(num_epochs):
        ...
        if args.DPSGD == "True":
            ...
            print(
                "\tTrain Epoch: [{}/{}] \t"
                "Loss: {:.6f} "
                "(ε = {:.2f}, δ = {})".format(epoch + 1, num_epochs, np.mean(values), epsilon, delta)
            )
        else:
            print(
                "\tTrain Epoch: [{}/{}] \t"
        ...
        recommendations, KLD, average_ndcg_normal=generate_recommendations(model, test_user_item_dict, user_indices, item_mapping, top_k, interacted_distr,
        ...
        """print(f"accuracy train:{np.mean(Accuracy_train)}")
        accuracy_train.append(np.mean(Accuracy_train))"""
        ...
        if consecutive_epochs_without_improvement >= max_consecutive_epochs_without_improvement:
            print(
                "Stopping training due to lack of improvement for {} epochs.".format(max_consecutive_epochs_without_improvement))
    ...
    def evaluation_and_save():
        ...
        recommendations,calibrated_recommendations,average_ndcg_normal,average_ndcg_calibrated, KLDN, KLDC=generate_recommendations(model, test_user_item_dict, user_indices, item_mapping, top_k, interacted_distr,
        ...
        # evaluation per each user category
        ...
        def calculate_group_metrics(group_name, user_group, recommendations, calibrated_recommendations, train_data, item_mapping, test_user_item_dict, interacted_items_test, model):
            ...
            # Calculate KLD
            kld_normal = np.mean([Calibration.compute_kl_divergence(interacted_distr[user_id], Calibration.compute_genre_distr(user_group_recommendations[user_id])) for user_id in user_group])
            kld_calibrated = np.mean([Calibration.compute_kl_divergence(interacted_distr[user_id], Calibration.compute_genre_distr(user_group_recommendations_calibrated[user_id])) for user_id in user_group])
            ndcg_per_user_type=[]
        ...
        all_metrics.update({
            ...
            'model_name':'{}_{}'.format(model_name,ratio),
