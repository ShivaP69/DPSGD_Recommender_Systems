...
def PL(train_data, item_mapping, recommendations,users, model):
  ...
  with torch.no_grad():
      for u in users:
          ...
          history.append((y / (len(interacted_items[u])) if len(interacted_items[u])>0 else 0 ))
      GAP_Q_G = np.mean(gap)
...
def Diversity(recommendation_list, df):
    for u in recommendation_list:
        if len(recommendation_list[u]) == 0:
            print(f"There is no recommendations for user {u}")

    name_map={}
    for name in df['name'].unique():
        df_temp =df[df['name'] == str(name)].drop(['name'], axis=1).values.flatten().tolist()
        name_map[name]=np.asarray(df_temp)
    #print(name_map)
    ...
    for user_id, movie_list in recommendation_list.items():
        ...
        for i in range(len(movie_list)):
            for j in range(i + 1, len(movie_list)):
                ...
                #movie1_data = df[df['name'] == str(movie1)].drop(['name'], axis=1).values.flatten().tolist()
                #movie2_data = df[df['name'] == str(movie2)].drop(['name'], axis=1).values.flatten().tolist()
                movie1_data = name_map.get(str(movie1), np.empty((0, 0)))
                movie2_data = name_map.get(str(movie2), np.empty((0, 0)))

                if np.sum(movie1_data) > 0 and np.sum(movie2_data) > 0:
                    #normalization
                    #movie1_data= movie1_data/movie1_data.sum()
                    #movie2_data= movie2_data/movie2_data.sum()
...
def serendepity_group(user_group, test_user_item_dict, item_mapping, recommendations, genres_df):
    ...
    title_col = 'name'
    name_map={}
    for name in genres_df['name'].unique():
        df_temp =genres_df[genres_df['name'] == name].drop(columns=['name'])
        name_map[name]=np.asarray(df_temp.values)


    for u in user_group:
        if u in test_user_item_dict and len(test_user_item_dict[u]) > 0 and u in recommendations:
            interacted_items = test_user_item_dict[u]
            interacted_items = [index for index in interacted_items if index in item_mapping]
            interacted_items_test = [item_mapping[item_id] for item_id in interacted_items]

            R = len(recommendations[u])
            H = len(interacted_items_test)
            if R > 0 and H > 0:
                cosin_sim_sum = 0
                for rec_item in recommendations[u]:
                    for interacted_item in interacted_items_test:
                        #df1 = genres_df[genres_df['name'] == str(rec_item)].drop(columns=['name'])
                        #df2 = genres_df[genres_df['name'] == str(interacted_item)].drop(columns=['name'])
                        matrix1= name_map.get(str(rec_item), np.empty((0, 0)))
                        matrix2= name_map.get(str(interacted_item), np.empty((0, 0)))
    ...
    if serendepity:
        return np.mean(serendepity)
         ...
         print("There is a problem")  # or some appropriate default value
...
def type_of_user_total(item_mapping, popular_items,ratings):
    ...
    diverse=[]
    x={}
    users=ratings[user_col].tolist()
    users= np.unique(users)
    all_users =ratings.groupby(user_col)[item_col].apply(list).to_dict()
...
def genres_features(df_item, result_dict):
    item_col = 'business_idx'
    title_col = 'name'
    genre_col = 'categories'

    #all_genres = set(result_dict.keys())
    all_genres = set(result_dict['restaurants'])
    #print(all_genres)

    # Creating a dictionary to hold the genre data
    genre_data = {title: {genre: 0 for genre in all_genres} for title in df_item[title_col].unique()}

    for _, row in df_item.iterrows():
        item_title = row[title_col]
        item_genre = row[genre_col]

        # Check if 'categories' is a list or needs to be split
        genres_list = item_genre if isinstance(item_genre, list) else item_genre.split('|')

        # Ensure the genres are in the result_dict
        genres_list = [genre for genre in genres_list if genre in all_genres]

        if genres_list:
            genre_ratio = 1.0 / len(genres_list)
            for genre in genres_list:
                genre_data[item_title][genre] += genre_ratio

    # Creating DataFrame from the dictionary
    genres_df = pd.DataFrame.from_dict(genre_data, orient='index').reset_index()
    genres_df.rename(columns={'index': title_col}, inplace=True)
    #print("genres_df_columns", genres_df.columns)
...
def catalog_coverage(predicted: list, catalog: list) -> float:
  ...
  #print(f"predicted_flattened: {predicted_flattened}")
  #print(f"catalog: {catalog}")
  # keep valid items
  valid_prediction= [item for item in predicted_flattened if item in catalog]
  #print("valid_prediction: ", valid_prediction)
  L_predictions = len(set(valid_prediction))  # remove duplicates
  # output: precent (%)
...
def novelty(list_recommendation, traning_data, item_mapping, k):
    ...
    user_col = 'user_idx'
    #print(popular)
    u = len(traning_data[user_col].unique())
    mean_self_information = []
    ...
    for user_id in list_recommendation.keys():
        ...
        for item in list_recommendation[user_id]:
            if item in popular.keys():
                item_popularity = popular[item] / u  # how much popular among how many users
...
def DPF(recommendations, category_mapping):
    ...
    #total_exposure = exposure_I1 + exposure_I2
    #normalized_exposure_1 = exposure_I1 / total_exposure if total_exposure > 0 else 0
    #normalized_exposure_2 = exposure_I2 / total_exposure if total_exposure > 0 else 0
