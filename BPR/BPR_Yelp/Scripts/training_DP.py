...
from create_data_loader_DP import creat_data
import evaluation
import torch
import models
import pandas as pd
...
# Declare an array of strings, each string representing a set of layer sizes
...
def load_or_initialize_model(model, optimizer, model_name,load_model=False):
    ...
    if load_model:
        if os.path.isfile(checkpoint_path):
            ...
            print(f"No saved model found at {checkpoint_path}, starting from scratchf.")
...
def train(args):
    ...
    train_loader = creat_data(user_interacted_items, all_movieIds, batch_size, num_negatives=4)
    test_loader = creat_data(test_user_item_dict, all_movieIds, batch_size, num_negatives=4)

    # extracting user history and apply LDP to generate new positive set (new DP history)
    for u in user_interacted_items:
        positive_items = user_interacted_items[u]
        new_positive_samples = apply_dp(positive_items, all_movieIds,args.privacy) # Applying LDP
        user_interacted_items[u] = new_positive_samples

    # extract the name of the interacted items
    interacted_items = {}
    for u in user_interacted_items:
    ...
    # calculate the distribution of items in the history of the user
    interacted_distr = {}
   ...
   # extract the interacted items for the test users
    ...
    model_name = f"BPR_{ratio}_{args.privacy}_{batch_size}_Yelp"
    ...
    def evaluation_and_save():
        ...
        # By valid_distr_extraction, we can extract valid (relevant) items for calculating different metrics per each item category: eg. distribution of I1 items
...
if __name__ == "__main__":
    ...
    parser.add_argument('--latent_dim', type=int, default=8, help='latent_dim value')
