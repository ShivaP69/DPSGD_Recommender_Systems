...
def save_model(model, optimizer, noise_multiplier, epoch):
    model_path = model_path_template.format(noise_multiplier=noise_multiplier, epoch=epoch)
    torch.save({
        'epoch': epoch,
...
def load_model_if_exists(model, optimizer, noise_multiplier,load_model=False):
    if load_model:
        model_files = [f for f in os.listdir() if f.startswith(f"model_{noise_multiplier}_")]
...
def train(args):
    ...
    train_dataloader = DataLoader(DataProcess.MovieLensTrainDataset(train_data,all_movieIds), batch_size=batch_size, num_workers=1)
    test_dataloader = DataLoader(DataProcess.MovieLensTrainDataset(test_data,all_movieIds), batch_size=batch_size, num_workers=1)

    # defining the model
    model = models.SVD(num_users, num_items, latent_dim)
    # Choose an optimizer (e.g., Adam)
    optimizer = optim.Adam(model.parameters(), lr=initial_learning_rate)
    model, optimizer, start_epoch = load_model_if_exists(model, optimizer, args.noise_multiplier, args.load_model)

    # setting the configuration for applying DPSGD
    if args.DPSGD == 'True':
        model_name = 'DPSGD_SVD_Yelp'
        delta = 1e-5
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if (epoch + 1) % 50 == 0:
            if args.save_model:
                if args.DPSGD == 'True':
                    save_model(model, optimizer, args.noise_multiplier, epoch)
                else:
                    save_model(model, optimizer,'normal', epoch)
    ...
    def evaluation_and_save():
        ...
        #classes= ['normal', 'calibrated']
        """dfs = {"niche_user": pd.DataFrame(index= classes,columns=['Novelty', 'Diversity', 'Serendepity', 'MRR', 'PL', 'KLD']),
               "blockbuster": pd.DataFrame(index= classes, columns=['Novelty', 'Diversity', 'Serendepity', 'MRR', 'PL', 'KLD']),
               "diverse": pd.DataFrame(index= classes, columns=['Novelty', 'Diversity', 'Serendepity', 'MRR', 'PL', 'KLD'])}"""
        ...
        def calculate_group_metrics(group_name, user_group, recommendations, calibrated_recommendations, train_data, item_mapping, test_user_item_dict, interacted_items_test, model):
            ...
            kld_normal = np.mean([Calibration_Func.compute_kl_divergence(interacted_distr[user_id],
                                                                         Calibration_Func.compute_genre_distr(user_group_recommendations[user_id])) for user_id in user_group if user_id in user_group_recommendations])
        ...
        results_file = f'SVD_Yelp_DPSGD.csv'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--noise_multiplier',default=0.1 ,type=float)
    parser.add_argument('--DPSGD', type=str, default="False", help='True or False for DPSGD')
    parser.add_argument('--trained_model', default=False, type=bool)
