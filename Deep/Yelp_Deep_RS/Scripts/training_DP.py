...
def train(args):
    ...
    for u in user_interacted_items:
        ...
        new_positive_samples = apply_dp(positive_items, all_movieIds,args.privacy) # apply DP
    ...
    train_dataloader = DataLoader(MovieLensTrainDataset(user_interacted_items, all_movieIds), batch_size=batch_size, num_workers=1)
    val_dataloader = DataLoader(MovieLensTrainDataset(test_user_item_dict, all_movieIds), batch_size=batch_size, num_workers=1)
    # defining the model
    model = NeuMF(num_users, num_items, mf_dim=mf_dim, mlp_layer_sizes=mlp_layer_sizes, dropout=dropout)
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=initial_learning_rate)
    model_name=f"dp_Deep_RS_NeuMF_{ratio}_{args.privacy}_{args.batch_size}_Yelp"
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in train_dataloader:
            ...
            # loss= model.calculate_loss(predicted_labels, labels)
            # loss = torch.nn.functional.binary_cross_entropy(predicted_labels, labels.view(-1, 1).float())
        ...
        print("Starting of recommendations creation")
    ...
    # plot_result(train_losses, test_losses, save_path=f"learning_rate:{initial_learning_rate}mf:{mf_dim}.png")
    ...
    def evaluation_and_save():
        ...
        item_col = 'business_idx'
        top_k = 10
        test_user_item_dict = test_data.groupby('user_idx')['business_idx'].apply(list).to_dict()
        ...
        Calibrated_PL = evaluation.PL(train_data, item_mapping, calibrated_recommendations, list(calibrated_recommendations.keys()),
                                model)
        ...
        # calculate metrics for different categories of items
        ...
        # calculate metrics for different type of users
        ...
        for group_name, user_group in user_groups.items():
            group_metrics = calculate_group_metrics(group_name, user_group, recommendations, calibrated_recommendations,
                                                    train_data, item_mapping, test_user_item_dict, interacted_items_test, model)
        ...
        results_file = "Deep_Yelp_LDP.csv"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mlp_layer_sizes', type=str, default='32,16,8', help='MLP layer sizes separated by commas')
    ...
    parser.add_argument('--Plot',type=bool, default=True, help='True or False for Plotting')
