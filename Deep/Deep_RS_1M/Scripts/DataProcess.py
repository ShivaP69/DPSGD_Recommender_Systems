...
class MovieLensTrainDataset(Dataset):
    ...
    def get_dataset(self, ratings, all_movieIds):
        ...
        num_negatives =4
        # For each positive feedback, add 1 as a label, and also pick 4 random negative items and add 0 as a label
        for u, i in user_item_set:
            users.append(u)
            items.append(i)
            labels.append(1)
        ...
        return torch.tensor(users), torch.tensor(items), torch.tensor(labels) # we have tensors such as (user_id, item, label)
