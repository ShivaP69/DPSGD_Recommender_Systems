...
def calculate_recall(recommended_items, interacted_items): # for one user
    """
...
def genres_features(df_item):
    """
        Create a DataFrame with genre features for each item.

        Parameters:
            df_item: DataFrame containing item data with 'title', 'genres', and 'movieId' columns
        Returns:
            DataFrame: Matrix with titles and binary genre indicators
    """
    title_col = 'title'
    genre_col = 'genres'
    item_col = 'movieId'
    ...
    genres_df = genres_df.astype({'title': 'str', **{genre: 'float64' for genre in all_genres}})
